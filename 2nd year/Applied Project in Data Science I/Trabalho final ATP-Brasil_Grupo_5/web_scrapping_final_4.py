...
for player in unique_players:
    ...
    if pd.isna(player_rows['Height'].iloc[0]):
        ...
        # Encontra o elemento que contém a altura em cm
        altura_cm = soup.find('span', {'class': 'table-height-cm-wrapper'})

        # Verifica se a altura foi encontrada
        if altura_cm is not None:
            
            altura_cm = altura_cm.text.strip()

            # Extrai apenas o valor numérico da altura em cm
            altura_num = re.search(r'\d+', altura_cm).group()

            # Atualiza o valor da altura para todos os registros associados a este jogador
            df_brasil.loc[df_brasil['LinkPlayer'] == player, 'Height'] = altura_num
            
        else:
            # Define a altura como 0 caso não seja encontrada para todos os registros associados a este jogador
            df_brasil.loc[df_brasil['LinkPlayer'] == player, 'Height'] = np.nan
"""
    ...
    for index, row in player_rows.iterrows():
        ...
        if pd.isna(player_rows['Height'].iloc[0]):
          ...
          #  print(url)
            ...
            height = soup.find("div", {"class": "date"}, string=lambda s: "Height / Weight" in s)
            
            if height is not None:
                height = height.text.split(':')[1].split(',')[0].split('/')[0].strip()
                height_num = re.search(r'\d+', height).group()
                df_brasil.loc[df_brasil['LinkPlayer'] == player, 'Height'] = height_num
...
for player_name in player_names:
    df_brasil.loc[(df_brasil['Oponent'] == player_name) 
                  & (df_brasil['HeightOpponent'].isnull()), 'HeightOpponent'] = df_brasil.loc[df_brasil['PlayerName'] == player_name, 'Height'].values[0]
