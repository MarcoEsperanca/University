...
for player in unique_players:
    ...
    if pd.isna(player_rows['Location'].iloc[0]):
...
for player in unique_players:
    ...
    for index, row in player_rows.iterrows():
        ...
        if pd.isna(player_rows['BornCountry'].iloc[0]):
          ...
          #  print(url)
            ...
            born_country = soup.find('div', {'class': 'date'})
            
            if born_country is not None:
                born_country = born_country.text.split(': ')[1]
                df_brasil.loc[df_brasil['LinkPlayer'] == player, 'BornCountry'] = born_country
                                               
            else:
                df_brasil.loc[df_brasil['LinkPlayer'] == player, 'BornCountry'] = np.nan
...
for player in unique_players:
    ...
    # Verifica se a mão está faltando (NaN) para o jogador atual
    if pd.isna(player_rows['BornCity'].iloc[0]):
        ...
        if all_table_values is not None:
            ...
            born_city = all_table_values[0].text.split(",")[0].strip()

            # Atualiza o valor da mão para todos os registros associados a este jogador
            df_brasil.loc[df_brasil['LinkPlayer'] == player, 'BornCity'] = born_city
        
        else:
            # Define a mão como 0 caso não seja encontrada para todos os registros associados a este jogador
            df_brasil.loc[df_brasil['LinkPlayer'] == player, 'BornCity'] = np.nan               
...
for player in unique_players:
    ...
    if pd.isna(player_rows['BornCountry'].iloc[0]):
        ...
        if all_table_values is not None:
            ...
            born_country = all_table_values[0].text.split(",")[-1].strip()
...
for player_name in player_names:
    df_brasil.loc[(df_brasil['Oponent'] == player_name) 
                  & (df_brasil['BornCityOpponent'].isnull()), 'BornCityOpponent'] = df_brasil.loc[df_brasil['PlayerName'] == player_name, 'BornCity'].values[0]


for player_name in player_names:
    df_brasil.loc[(df_brasil['Oponent'] == player_name) 
                  & (df_brasil['BornCountryOpponent'].isnull()), 'BornCountryOpponent'] = df_brasil.loc[df_brasil['PlayerName'] == player_name, 'BornCountry'].values[0]                
