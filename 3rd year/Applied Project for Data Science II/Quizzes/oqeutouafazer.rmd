...
split <- sample(nrow(condutoresMaisLimpo), nrow(condutoresMaisLimpo) * 0.7)
...
A Árvore de Regressão é constituída por  **7** nós folha;  a Residual Deviance associada ao modelo sobre a amostra de teste é **733866.6**; o erro quadrático de previsão, relativo a Idade.Veiculo, para a primeira observação do conjunto teste é **38.1603**. Para reduzir a complexidade do modelo em árvore o valor do argumento mindev da function tree deve ser alterado para **TODO**(selecione um dos seguintes valores: 0.01; 0.0001).
...
residualDeviance <- sum((teste$Idade.Veiculo - predicao)^2)
residualDeviance
```
